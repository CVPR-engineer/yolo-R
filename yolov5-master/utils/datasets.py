...
def flatten_recursive(path=DATASETS_DIR / 'coco129'):
...
def extract_boxes(path=DATASETS_DIR / 'coco129'):  # from utils.datasets import *; extract_boxes()
...
def autosplit(path=DATASETS_DIR / 'coco129/images', weights=(0.9, 0.1, 0.0), annotated_only=False):
...
def dataset_stats(path='coco129.yaml', autodownload=False, verbose=False, profile=False, hub=False):
    """ Return dataset statistics dictionary with images and instances counts per split per class
    To run in parent directory: export PYTHONPATH="$PWD/yolov5"
    Usage1: from utils.datasets import *; dataset_stats('coco129.yaml', autodownload=True)
