...
def plot_one_box(xyxy, resultImg, label, color, line_thickness=3):
    """
    在给定图像上绘制一个边界框以及对应的标签。

    参数:
    xyxy (list或tuple): 边界框的坐标，格式为 [x1, y1, x2, y2]，其中 (x1, y1) 是左上角坐标，(x2, y2) 是右下角坐标。
    resultImg (numpy.ndarray): 要绘制边界框的图像，格式为OpenCV的图像数组（通常是BGR格式）。
    label (str): 边界框上要显示的标签文本，例如 "class_name confidence"。
    color (list或tuple): 绘制边界框的颜色，格式为 (B, G, R)，对应OpenCV的颜色通道顺序。
    line_thickness (int, 可选): 边界框线条的粗细程度，默认值为3。

    返回:
    numpy.ndarray: 绘制好边界框和标签后的图像。
    """
    # 将坐标解包
